...
function toggle(prog, vert, horiz, width, height, sticky, screen)
    ...
    if not dropdown[prog][screen] then
        spawnw = function (c)
            ...
            if     vert == "bottom" then y = screengeom.height +
screengeom.y - height
            elseif vert == "center" then y =
screengeom.y+(screengeom.height-height)/2
        ...
        if c.hidden then
            ...
            if vert  == "center" then awful.placement.center_vertical(c)
end
            if horiz == "center" then awful.placement.center_horizontal(c)
end
            c.hidden = false
            c:raise()
            capi.client.focus = c
